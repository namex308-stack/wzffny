...
use Asciisd\Kashier\Validation\JsonValidator;
...
class KashierModel
{
     ...
     *
     * @var KashierKey $credential
     */
    protected static KashierKey $credential;

    /**
     * Sets Credential
     *
     * @param KashierKey $credential
     * @deprecated Pass ApiContext to create/get methods instead
     */
    public static function setCredential(KashierKey $credential): void
    {
     ...
     */
    public function __construct(array|string $data = null)
    {
        switch (gettype($data)) {
            case 'NULL':
                break;
     ...
     * @return array|KashierModel|null
     * @throws KashierConfigurationException
     */
    public static function getList($data): KashierModel|array|null
    {
        // Return Null if Null
        if ($data === null) {
            return null;
     ...
     */
    private function _convertToArray($param): KashierModel|array
    {
        $ret = array();
     ...
     * @throws ReflectionException
     */
    public function fromArray($arr): KashierModel
    {
        if (! empty($arr)) {
            // Iterate over each element in array
            foreach ($arr as $k => $v) {
                // If the value is an array, it means, it is an object after conversion
                if (is_array($v) && ($className = ReflectionUtil::getPropertyClass(get_class($this), $k)) !== null && strtolower($className) !== 'array') {
                    ...
                    if (empty($v)) {
                    ...
                    } elseif (ArrayUtil::isAssocArray($v) && strtolower($className) !== 'array') {
                        /** @var self $o */
                        $o = new $className();
                        $o->fromArray($v);
                        $this->assignValue($k, $o);
                        ...
                        foreach ($v as $nk => $nv) {
                            if (is_array($nv) && strtolower($className) !== 'array') {
                                $o = new $className();
                                $o->fromArray($nv);
                                $arr[$nk] = $o;
                            } else {
     ...
     * @throws ReflectionException
     */
    public function fromJson($json): KashierModel|static
    {
     ...
     *
     * @return KashierModel|array
     * @throws KashierConfigurationException
     * @throws ReflectionException
     */
    public function toArray(): KashierModel|array
    {
