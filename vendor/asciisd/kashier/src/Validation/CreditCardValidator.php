...
class CreditCardValidator
{
    ...
    public static function validCreditCard($number, $type = null)
    {
        ...
        if (array_key_exists($type, self::$_cards) && self::validCard($number, $type)) {
            return array(
                'valid' => true,
                'number' => $number,
                'type' => $type,
    ...
    {
        return (is_numeric($cvc) && array_key_exists($type, self::$_cards) && self::validCvcLength($cvc, $type));
    }
    ...
    {
        foreach (self::$_cards as $type => $card) {
    ...
    {
        return preg_match(self::$_cards[$type]['pattern'], $number);
    }

    protected static function validLength($number, $type)
    {
        foreach (self::$_cards[$type]['length'] as $length) {
    ...
    {
        foreach (self::$_cards[$type]['cvcLength'] as $length) {
